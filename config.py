...
OPTIMIZATION_CONFIG = {
    ...
    "fast": {
        "num_iterations": 200,
        "learning_rate": 0.1,
        "lambda_vgg": 0.5,           # Peso espacial VGG (igualado a CLIP)
        "lambda_tv": 0.005,          # Regularización Total Variation (suavidad)
        "langevin_noise_interval": 20,
        "langevin_noise_scale": 0.005,
        "quantize_interval": 50,     # Proyectar al manifold VQGAN cada N iter
    ...
    "standard": {
        "num_iterations": 500,
        "learning_rate": 0.05,
        "lambda_vgg": 0.5,           # CORREGIDO: 0.1→0.5 para forzar estructura espacial
        "lambda_tv": 0.01,           # Regularización Total Variation
        "langevin_noise_interval": 10,
        "langevin_noise_scale": 0.005,  # Escala máxima (se reduce con schedule)
        "quantize_interval": 50,     # Anclar al codebook VQGAN cada 50 iter
...
EVALUATION_CONFIG = {
    # Métricas
    "compute_metrics": True,  # Ejecutar evaluation.py tras cada reconstrucción
    "metrics": ["ssim", "pixcorr", "psnr", "lpips", "clip", "pairwise"],
    # Comparación con ground truth: se extrae de target_images.pkl
    "compare_with_original": True,
    "original_images_path": (
        REPOS["mental_img_recon"] / "ref_images" / "target_images.pkl"
    ),
    # Reporte
    "export_csv": True,
    "export_html": True,
...
def validate_paths():
    ...
    if not DATA_DIRS["decoded_features"].exists():
        errors.append(
            f"Directorio de features no encontrado: {DATA_DIRS['decoded_features']}"
        )
...
def get_mean_clip_path():
    ...
    return (
        DATA_DIRS["mean_features"] / "CLIP_ViT-B_32" / "lastLayer" / "meanFeature_.mat"
    )
